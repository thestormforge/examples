...
You must have a Kubernetes cluster. While this example will run on Minikube, we highly recommend using a cluster with 2 nodes, 8vCPUs (4 on each node) and 16GB of memory (8 on each node).
...
* `experiment-rbac.yaml` : RBAC resources required for the experiment. A supplement to (and automatically generated from) the `experiment.yaml` file.
* `experiment.yaml` : The StormForge Optimize Pro experiment file used to optimize a kubernetes workload.
* `postgres.yaml` : The resources for the Postgres application itself.
